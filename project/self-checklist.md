...
| 2  | Endpoint `/predict (/detox)` использует **реальную модель**, а не заглушку      |         ✅     | Например: `src/service/`, `src/models/`                             |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                                 |         ✅     | Например: `notebooks/EDA`, `notebooks/model_testing`, `report.md`   |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**               |         ✅     | Например: `notebooks/model_testing`, `report.md`                    |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`                   |         ✅     | Например: `src/data/`, `src/models/`, `src/service/`, `src/features` |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker              |       ✅        | Например: шаги в `README.md`                        |
