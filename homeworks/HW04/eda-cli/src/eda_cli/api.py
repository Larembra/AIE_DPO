...
@app.post(
    ...
    tags=["quality"],
    summary="Расширенная оценка качества по CSV-файлу с использованием всех флагов качества EDA-ядра"
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> Flags:
    """
        Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
        (summarize_dataset + missing_table + compute_quality_flags)
        и возвращает оценку качества данных по ВСЕМ флагам качества.
    """
    start = perf_counter()
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    ...
    if df.empty:
        raise HTTPException(status_code=400, detail="CSV-файл не содержит данных: (пусто DataFrame).")
