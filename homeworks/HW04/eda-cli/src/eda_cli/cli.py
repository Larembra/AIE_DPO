...
def report(
    ...
    min_missing_share: float = typer.Option(0.01, help="Порог доли пропусков, выше которого колонка считается проблемной и попадает в отдельный список в отчёте (report.md).")
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть колонки, где все значения одинаковы: **{quality_flags['has_constant_columns']}**\n")
        ...
        f.write(f"- Дупликаты user_id: **{quality_flags['has_suspicious_id_duplicates']}**\n")
        f.write(f"- Столбцы с большой долей нулей: **{quality_flags['has_many_zero_values']}**\n\n")

        f.write("## Колонки\n\n")
        f.write("### Проблемные колонки с большой долей пропусков (тут будет пусто если таких нет)\n\n")
        for col_name, row in missing_df.iterrows():
            if row['missing_share'] > min_missing_share:
                f.write(f"- {col_name}: {row['missing_share']:.2%}\n")
