...
def compute_quality_flags(summary: DatasetSummary, df: pd.DataFrame, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    flags["has_constant_columns"] = any(df[i].nunique()==1 for i in df.columns)

    cat_cols = df.select_dtypes(include=['object', 'category']).columns
    flags["has_high_cardinality_categoricals"] = any(
        df[col].nunique() / summary.n_rows > 0.5 for col in cat_cols
    )

    flags["has_suspicious_id_duplicates"] = df["user_id"].nunique() != summary.n_rows

    number_cols = df.select_dtypes(include=['int64', 'float64']).columns
    flags["has_many_zero_values"] = any((df[col]==0).sum() / summary.n_rows > 0.33 for col in number_cols)
